...
def recommend():
    user_input = request.form['input']  # Lấy dữ liệu từ ô nhập liệu
    ...
    weight_description_sent2vec = 0.6
...
def recommend_abstract():
    user_input_abstract = request.form['input_abstract']  # Lấy dữ liệu từ ô nhập liệu cho abstract

    start_time = time.time()

    # Mã hóa input
    input_embedding = model.encode([user_input_abstract])
    ...
    return render_template(
        ...
        execution_time=execution_time,
        num_results=num_results,
        user_input=user_input_abstract,
        borda_scores=borda_scores
    )
...
def find_conference(article_id):
    ...
    return render_template('results.html', results=top_results_with_distances, execution_time=execution_time, num_results=num_results, user_input=title, borda_scores=borda_scores)
...
def filter_results():
    ...
    # Tìm kiếm hội thảo trước khi lọc
    input_embedding = model.encode([user_input])
    k = 10
    ...
    return render_template('results.html', results=filtered_results, num_results=num_results, user_input=user_input, selected_rank=selected_rank, selected_field=selected_field, borda_scores=borda_scores)
